# encoding: utf-8
require 'sliday_backup/config/dsl'
require 'sliday_backup/config/helpers'
  ...
  module Config
    ...
    class << self
      ...
      def load(options = {})
        ...
        version = SlidayBackup::VERSION.split('.').first
        unless config =~ /^# SlidayBackup v#{ version }\.x Configuration$/
          raise Error, <<-EOS
            Invalid Configuration File
            The configuration file at '#{ config_file }'
            does not appear to be a SlidayBackup v#{ version }.x configuration file.
            If you have upgraded to v#{ version }.x from a previous version,
            you need to upgrade your configuration file.
            Please see the instructions for upgrading in the SlidayBackup documentation.
          EOS
        end
      ...
      def reset!
        ...
        @root_path = File.join(File.expand_path(ENV['HOME'] || ''), 'SlidayBackup')
