...
module SlidayBackup
  class CLI < Thor
    ...
    #   3: A fatal error caused SlidayBackup to exit.
    ...
    method_option :log_path,
                  ...
                  :type     => :string,
                  :default  => '',
                  :desc     => "Path to store SlidayBackup's log file."
    ...
    method_option :logfile,
                  ...
                  :default  => true,
                  :banner   => '',
                  :desc     => "Enable SlidayBackup's log file."
        ...
        when 2
          ...
          unless models.empty?
            Logger.info Error.new(<<-EOS)
              SlidayBackup will now continue...
    ...
    desc 'generate:model', "Generates a SlidayBackup model file."

    long_desc <<-EOS.gsub(/^ +/, '')
      Generates a SlidayBackup model file.
    ...
    method_option :trigger,
                  ...
                  :required => true,
                  :type     => :string,
                  :desc     => 'Trigger name for the SlidayBackup model'

    method_option :config_file,
                  :type     => :string,
                  :desc     => 'Path to your SlidayBackup configuration file'

    # options with their available values
    %w{ databases storages syncers encryptor compressor notifiers }.each do |name|
      path = File.join(SlidayBackup::TEMPLATE_PATH, 'cli', name)
    ...
    define_method 'generate:model' do
      ...
      if Helpers.overwrite?(model_file)
        File.open(model_file, 'w') do |file|
          file.write(SlidayBackup::Template.new({:options => opts}).result('cli/model'))
        end
    ...
    desc 'generate:config', 'Generates the main SlidayBackup configuration file'

    long_desc <<-EOS.gsub(/^ +/, '')
      Path to the SlidayBackup configuration file to generate.
    ...
    method_option :config_file,
                  :type => :string,
                  :desc => 'Path to the SlidayBackup configuration file to generate.'
    ...
    define_method 'generate:config' do
      ...
      if Helpers.overwrite?(config_file)
        File.open(config_file, 'w') do |file|
          file.write(SlidayBackup::Template.new.result('cli/config'))
        end
    ...
    ##
    # [Version]
    # Returns the current version of the SlidayBackup gem
    map '-v' => :version
    desc 'version', 'Display installed SlidayBackup version'
    def version
      puts "SlidayBackup #{ SlidayBackup::VERSION }"
    end
    ...
    module Helpers
      class << self
        ...
        def is_backup_error?(error)
          error.class.ancestors.include? SlidayBackup::Error
        end
