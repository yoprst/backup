...
module SlidayBackup
  class Logger
    class Logfile
      ...
      class Options
        ##
        # Enable the use of SlidayBackup's log file.
        #
        # While not necessary, as this is +true+ by default,
        # this may also be set on the command line using +--logfile+.
        #
        # The use of SlidayBackup's log file may be disabled using the
        ...
        ##
        # Path to directory where SlidayBackup's logfile will be written.
        #
        # This may be given as an absolute path, or a path relative
        # to SlidayBackup's +--root-path+ (which defaults to +~/SlidayBackup+).
        #
        ...
        ##
        # SlidayBackup's logfile in which backup logs can be written
        #
      ...
      def setup_logfile
        ...
        if path.empty?
          path = File.join(SlidayBackup::Config.root_path, 'log')
        elsif path != File.expand_path(path)
          path = File.join(SlidayBackup::Config.root_path, path)
        end
