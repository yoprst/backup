  ...
  module Notifier
    class FlowDock < Base
      ...
      def default_tags(status)
        case status
        when :success then ['#SlidayBackupSuccess']
        when :warning then ['#SlidayBackupWarning']
        when :failure then ['#SlidayBackupFailure']
        end
      end
      ...
      #set default source
      def default_source
        "SlidayBackup #{ model.label }"
      end

      # set default subject
      def default_subject
        'SlidayBackup Notification'
      end
