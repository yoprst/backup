...
describe Encryptor::GPG do
    ...
    create_model :my_backup, <<-EOS
      SlidayBackup::Model.new(:my_backup, 'a description') do
        ...
        encrypt_with GPG do |encryptor|
          ...
          encryptor.gpg_homedir = SlidayBackupSpec::GPG_HOME_DIR

          encryptor.keys = {
            # backup03 public key (email as identifier)
            'backup03@foo.com' => SlidayBackupSpec::GPGKeys[:backup03][:public],
            # backup04 public key (long key id as identifier)
            '0F45932D3F24426D' => SlidayBackupSpec::GPGKeys[:backup04][:public]
          }
    ...
    create_model :my_backup, <<-EOS
      SlidayBackup::Model.new(:my_backup, 'a description') do
        ...
        encrypt_with GPG do |encryptor|
          ...
          encryptor.keys = {
            'backup02@foo.com' => SlidayBackupSpec::GPGKeys[:backup02][:public],
          }
  ...
  specify ':symmetric mode' do
    ...
    log_messages = SlidayBackup::Logger.messages.map(&:formatted_lines).flatten.join
  ...
  def gpg_encryptor
    @gpg_encryptor ||= SlidayBackup::Encryptor::GPG.new do |gpg|
      gpg.gpg_homedir = SlidayBackupSpec::GPG_HOME_DIR
    end
  end

  def clean_homedir
    FileUtils.rm_rf SlidayBackupSpec::GPG_HOME_DIR
  ...
  def setup_homedir(key_type, users)
    ...
    users.each do |user|
      keyid = gpg_encryptor.send(:import_key,
        user, SlidayBackupSpec::GPGKeys[user][key_type]
      )
      unless keyid == SlidayBackupSpec::GPGKeys[user][:long_id]
        warn SlidayBackup::Logger.messages.map(&:lines).flatten.join("\n")
  ...
  def decrypt(path, passphrase = nil)
    ...
    if File.exist?(outfile)
      expect( SlidayBackupSpec::TarFile.new(outfile) ).to match_manifest(%q[
