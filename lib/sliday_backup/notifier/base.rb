...
module SlidayBackup
  module Notifier
    ...
    class Base
      ...
      #
      # @yieldparam [model] SlidayBackup::Model
      ...
      ##
      # Return the notifier name, with SlidayBackup namespace removed
      def notifier_name
      ...
      def status_data_for(status)
        {
          :success => {
            :message => 'SlidayBackup::Success',
            :key => :success
          },
          :warning => {
            :message => 'SlidayBackup::Warning',
            :key => :warning
          },
          :failure => {
            :message => 'SlidayBackup::Failure',
            :key => :failure
          }
        }[status]
      end
